...
func main() {
	...
	output := []int64{}
	ic := LoadIntCode(scanner.Text(), FixedInput(), func(out int64) { output = append(output, out) })
	ic.RunToEnd()

	countBlock := 0
	for i := 0; i < len(output); i += 3 {
		picture[Coord{output[i], output[i+1]}] = output[i+2]
		if output[i+2] == 2 {
			countBlock++
		}
	}
	fmt.Println("Part One: ", countBlock)
}
